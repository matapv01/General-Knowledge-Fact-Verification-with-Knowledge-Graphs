...
def get_answer(qid: int, claim: str, gt_entity: str, KG: dict, max_tokens: int):
    ...
    sentence_divide_query = open_file('./meta_2hop_prompts/sentence_divide_prompt.txt').replace('<<<<CLAIM>>>>', claim).replace('<<<<ENTITY_SET>>>>', '['+entities[0]+']')
    ...
    for _ in range(5):
        try:
            ...
            print("[ERROR]", e)
            time.sleep(5)
    with open(f'./meta_generated_prompts/2hop/{qid}_sentence_divide.pickle', 'wb') as f:
    ...
    for one_rel in list(KG[gt_entity]):
        ...
        for one_tail in one_tails:
            all_relation_set += list(KG[one_tail])
    ...
    with open(f'./meta_generated_prompts/2hop/{qid}_final_evidence.pickle', 'wb') as f:
        pickle.dump(new_final_evidence, f)
    # save_file(f'./generated_prompts/{qid}_final_evidence.txt', str(new_final_evidence))

    #### Verification phase
    verify_prompt = open_file('./meta_2hop_prompts/verify_claim_with_evidence.txt').replace('<<<<CLAIM>>>>', claim).replace('<<<<EVIDENCE_SET>>>>', str(new_final_evidence))
    # print('Verifying...')
    for _ in range(5):
        try:
            response = client.chat.completions.create(
            ...
            save_file(f'./meta_generated_prompts/2hop/{qid}_verification_result.txt', verification)
...
if __name__ == "__main__":
    ...
    for qid, question in questions_dict.items():
        ...
        with open(f'./twohop_result.pickle', 'wb') as f:
