...
def claim_divider_parse_answer(answer):
...
def get_answer(qid: int, claim: str, gt_entity: str, KG: dict, max_tokens: int):
    ...
    sentence_divide_query = open_file('./meta_3hop_prompts/sentence_divide_prompt.txt').replace('<<<<CLAIM>>>>', claim).replace('<<<<ENTITY_SET>>>>', '['+entities[0]+']')
    ...
    for _ in range(5):
        try:
            ...
            # print("gpt output is ", divide_claim_result)
            
            divided_claim_list = claim_divider_parse_answer(divide_claim_result)
            # print(divided_claim_list)
            break
        except Exception as e:
            print("[ERROR]", e)
            time.sleep(5)
    with open(f'./meta_generated_prompts/3hop/{qid}_sentence_divide.pickle', 'wb') as f:
        pickle.dump(divided_claim_list, f)
    # save_file(f'./generated_prompts/{qid}_sentence_divide.txt', str(divided_claim_list))
    ...
    for divided_claim, corresponding_entity_set in divided_claim_list.items():
        if len(corresponding_entity_set) == 1 and len(corresponding_entity_set[0]) > 0:
            if len(list(KG[corresponding_entity_set[0]])) == 1:
                start_relation.append(one_hop_relation_set)
                continue
            else:
                relation_retrieval_query = open_file('./meta_3hop_prompts/relation_retrieval_prompt.txt').replace('<<<<SENTENCE>>>>', divided_claim).replace('<<<<RELATION_SET>>>>', str(one_hop_relation_set))
        else:
            relation_retrieval_query = open_file('./meta_3hop_prompts/relation_retrieval_prompt.txt').replace('<<<<SENTENCE>>>>', divided_claim).replace('<<<<RELATION_SET>>>>', str(new_all_relation_set))
    ...
    # print('corresponding entity set is', corresponding_entity_set)
    # print('start is ', start_relation)
    # print('extra is ', extra_relation)
    final_evidence = []
    ...
    # print('final_evidence is ', new_final_evidence)
    with open(f'./meta_generated_prompts/3hop/{qid}_final_evidence.pickle', 'wb') as f:
        pickle.dump(new_final_evidence, f)
    # save_file(f'./generated_prompts/{qid}_final_evidence.txt', str(new_final_evidence))

    #### Verification phase
    verify_prompt = open_file('./meta_3hop_prompts/verify_claim_with_evidence.txt').replace('<<<<CLAIM>>>>', claim).replace('<<<<EVIDENCE_SET>>>>', str(new_final_evidence))
    # print('Verifying...')
    for _ in range(5):
        try:
            response = client.chat.completions.create(
            ...
            save_file(f'./meta_generated_prompts/3hop/{qid}_verification_result.txt', verification)
...
if __name__ == "__main__":
    ...
    api_key = open_file('./openai_api_key.txt').strip()
    ...
    # with open(f'./threehop_result.pickle', 'rb') as f:
    ...
    for qid, question in questions_dict.items():
        ...
        with open(f'./threehop_result.pickle', 'wb') as f:
